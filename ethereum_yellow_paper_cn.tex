...
\title{以太坊：一种安全去中心化的通用交易账本 \\ {\smaller \textbf{Berlin 版本 \YellowPaperVersionNumber}}}
\author{
    原文作者：Dr. Gavin Wood, gavin@ethcore.io\\
    中文版翻译：万山(wanshan1024@163.com);高天露(tianlu.jorden.gao@gmail.com);杨镇(rivers.yang@icloud.com)
}
\begin{document}
...
这意味着在一个给定的时间（区块），系统中会有多个状态共存：一些节点相信一个区块包含权威的交易，其它节点则相信另外的区块包含权威的交易，其中就包含彻底不同或者不兼容的交易。必须要不惜一切代价避免这点，因为它会破坏整个系统信用。

我们使用了一个简化版的 GHOST 协议来达成共识，参见 \cite{cryptoeprint:2013:881}。我们会在第 \ref{ch:ghost} 章中详细说明。

有时会从一个特定区块高度启用新的协议。本文描述了协议的一个版本，Berlin 版本，如果要跟踪协议版本变化，需要查看这份文档的各个历史版本。下面是以太坊主网协议更新的区块编号：
\par
\begin{center}
\begin{tabular}{lr}
\toprule
...
有时候有人不同意协议的更新， 就会发生永久分叉。为了区别分叉的链，EIP-155 \cite{EIP-155} 引入了 chain ID 的概念，使用 $\beta$ 表示。对以太坊主网来说
\begin{equation}
  \linkdest{chain_id}
  \beta = 1
\end{equation}
...
\begin{description}
\item[type]\linkdest{tx_type}{} EIP-2718 交易类型； 以 $T_{\mathrm{x}}$ 表示.
\item[nonce]\linkdest{tx_nonce}{} 由交易发送者发出的的交易的数量，以 $T_{\mathrm{n}}$ 表示。
\item[gasPrice]\linkdest{tx_gas_price_T__p}{} 为执行这个交易所需要进行的计算步骤消耗的每单位 \textit{gas} 的价格，以 Wei 为单位，以 $T_{\mathrm{p}}$ 表示。
\item[gasLimit]\linkdest{tx_gas_limit_T__g}{} 用于执行这个交易的最大 gas 数量。这个值须在交易开始前设置，且设定后不能再增加，由 $T_{\mathrm{g}}$ 表示。
\item[to]\linkdest{tx_to_address_T__t}{} 160 位的消息调用接收者地址；对与合约创建交易，用 $\varnothing$ 表示 $\mathbb{B}_0$ 的唯一成员。此字段以 $T_{\mathrm{t}}$ 表示
...
EIP-2930 (type 1) 交易还包含：
\begin{description}
\item[accessList]\linkdest{tx_access_list}{} 要预热的访问条目列表，以 $T_{\mathbf{A}}$ 表示.
\linkdest{access_list_entry}{}每一个访问条本列表 $E$ 是一个由账户地址和存储 key 列表组成的元组： $E \equiv (E_{\mathrm{a}}, E_{\mathbf{s}})$. 
\item[chainId]\linkdest{tx_chain_id}{} Chain ID; 以 $T_{\mathrm{c}}$ 表示. 需要和网络 chain ID 保持一致 \hyperlink{chain_id}{$\beta$}.
\item[yParity]\linkdest{tx_y_parity}{} 签名 Y parity， 以 $T_{\mathrm{y}}$ 表示.
\end{description}

旧类型交易不包含 \textbf{accessList} ($T_{\mathbf{A}}=()$), 针对旧类型交易把 \textbf{chainId} 和 \textbf{yParity} 组合为了一个值:
\begin{description}
\item[w]\linkdest{T__w}{} 一个标量值，由 Y parity 和 chain ID 编码而成，以 $T_{\mathrm{w}}$ 表示.
$T_{\mathrm{w}} = 27 + T_{\mathrm{y}}$ 或者 $T_{\mathrm{w}} = 2\beta + 35 + T_{\mathrm{y}}$ (见 EIP-155 \cite{EIP-155}).
\end{description}
...
\hypertarget{block_reward_R__block}{}\linkdest{R__block} 我们定义区块奖励的单位是 Wei:
\begin{equation}
